<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=180&section=header&text=Joel%20Smith%20M&fontSize=42&fontColor=fff&animation=twinkling&fontAlignY=32&desc=AI%20%26%20Data%20Science%20Engineer%20%7C%20Building%20Intelligent%20Systems%20for%20Real-World%20Impact&descAlignY=62&descSize=16" width="100%"/>

<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=26&duration=3000&pause=1000&color=70A5FD&center=true&vCenter=true&width=750&lines=AI+%26+Data+Science+Engineer+%F0%9F%A4%96;Full+Stack+%7C+Backend+%7C+NLP+Developer+%F0%9F%9A%80;Hackathon+Finalist+%7C+Innovator+%7C+Builder+%F0%9F%8F%86;District+Handball+%7C+State+Wushu+Athlete+%F0%9F%A5%8B)](https://git.io/typing-svg)

[![Profile Views](https://komarev.com/ghpvc/?username=joelsmith-00&style=flat-square&color=70a5fd&label=Profile+Views)](https://github.com/joelsmith-00)
[![GitHub followers](https://img.shields.io/github/followers/joelsmith-00?style=flat-square&color=bf91f3&logo=github&logoColor=white)](https://github.com/joelsmith-00)

</div>

---

## 👨‍💻 About Me

<img align="right" width="380" src="https://raw.githubusercontent.com/abhisheknaiidu/abhisheknaiidu/master/code.gif"/>

```python
class JoelSmithM:
    def __init__(self):
        self.name         = "Joel Smith M"
        self.location     = "Tamil Nadu, India 🇮🇳"
        self.degree       = "B.Tech – Artificial Intelligence & Data Science"
        self.college      = "Karunya Institute of Technology and Sciences"

        self.stack = [
            "Python", "FastAPI", "MERN Stack",
            "Machine Learning", "NLP", "Data Analysis",
            "REST APIs", "IoT Integration"
        ]

        self.currently_learning = [
            "Advanced Deep Learning",
            "LLM Fine-tuning & RAG Pipelines",
            "Cloud-Native AI Deployment (AWS)"
        ]

        self.fun_fact = (
            "I'm a district-level Handball player 🤾 "
            "AND a state-level Wushu martial artist 🥋"
        )

    def motto(self) -> str:
        return "Build AI that doesn't just compute — it understands. 🧠"

    def highlights(self) -> list:
        return [
            "🏆 3rd Prize – Mindcraft International Tech Expo (LUMI AI)",
            "🚀 Incubation Project: Sensory-Aware Wearable for Disabled Children",
            "👨‍🏫 Mentored 5 first-year students through AI & backend dev",
            "🔗 Qualified Round 2 – Smart India Hackathon & AI for Bharat"
        ]

me = JoelSmithM()
print(me.motto())
```

<br clear="right"/>

---

## 🛠️ Tech Stack

### 🧠 Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)

### ⚙️ Frameworks & Libraries
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-000000?style=flat-square&logo=express&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=flat-square&logo=scikitlearn&logoColor=white)

### ☁️ Cloud & DevOps
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)

### 🗄️ Databases
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)

### 🔧 Tools & Platforms
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=flat-square&logo=visualstudiocode&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=flat-square&logo=postman&logoColor=white)
![Raspberry Pi](https://img.shields.io/badge/Raspberry_Pi-A22846?style=flat-square&logo=raspberrypi&logoColor=white)
![Cisco](https://img.shields.io/badge/Cisco-1BA0D7?style=flat-square&logo=cisco&logoColor=white)

---

## 📊 GitHub Stats

<div align="center">

<a href="https://github.com/joelsmith-00">
  <img height="170" src="https://github-readme-stats.vercel.app/api?username=joelsmith-00&show_icons=true&theme=tokyonight&hide_border=true&include_all_commits=true&count_private=true&bg_color=0d1117&title_color=70a5fd&icon_color=bf91f3&text_color=c9d1d9&border_radius=10"/>
</a>
<a href="https://github.com/joelsmith-00">
  <img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=joelsmith-00&layout=compact&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=70a5fd&text_color=c9d1d9&langs_count=8&border_radius=10"/>
</a>

</div>

---

## 🔥 Streak Stats

<div align="center">

[![GitHub Streak](https://streak-stats.demolab.com?user=joelsmith-00&theme=tokyonight-duo&hide_border=true&background=0D1117&ring=70A5FD&fire=BF91F3&currStreakLabel=70A5FD&sideLabels=38BDAE&dates=8B949E&currStreakNum=C9D1D9&sideNums=C9D1D9&stroke=0D1117&border_radius=10)](https://git.io/streak-stats)

</div>

---

## 📈 Activity Graph

<div align="center">

[![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=joelsmith-00&theme=tokyo-night&bg_color=0d1117&color=70a5fd&line=bf91f3&point=38bdae&area=true&hide_border=true)](https://github.com/joelsmith-00)

</div>

---

## 🏆 Trophy Wall

<div align="center">

[![Trophies](https://github-profile-trophy.vercel.app/?username=joelsmith-00&theme=tokyonight&no-frame=true&no-bg=true&row=1&column=7&margin-w=4)](https://github.com/joelsmith-00)

</div>

---

## 💼 Work Experience

<details>
<summary><b>🏢 Gateway Software Solutions — Full Stack Developer Intern</b> &nbsp;|&nbsp; 2024 &nbsp;|&nbsp; Tamil Nadu, India</summary>

<br/>

> ![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white) ![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black) ![REST API](https://img.shields.io/badge/REST_API-009688?style=flat-square&logo=fastapi&logoColor=white) ![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)

- Designed and developed **frontend-backend integrated** web applications with responsive UI components using React.js and Node.js
- Built and tested **REST API endpoints** for data exchange, improving backend response efficiency across multiple modules
- Managed **database schema design and query optimization** using MongoDB, reducing data retrieval time significantly
- Participated in **full deployment lifecycle** — from local development to staging environments — gaining hands-on production experience

</details>

<details>
<summary><b>🏢 CodeC Technologies — MERN Stack Development Intern</b> &nbsp;|&nbsp; 2024 &nbsp;|&nbsp; Tamil Nadu, India</summary>

<br/>

> ![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white) ![Express](https://img.shields.io/badge/Express.js-000000?style=flat-square&logo=express&logoColor=white) ![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black) ![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white)

- Built **full-stack MERN applications** from scratch, handling routing, state management, and RESTful API integration end-to-end
- Developed **reusable React components** and integrated them with Express.js backend services for seamless data flow
- Implemented **authentication and authorization** flows and session handling for secure user management
- Collaborated in an **agile team environment**, contributing to sprint planning, code reviews, and iterative feature development

</details>

<details>
<summary><b>🔬 Karunya Institute — Incubation Project Lead</b> &nbsp;|&nbsp; 2024–Present &nbsp;|&nbsp; Tamil Nadu, India</summary>

<br/>

> ![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white) ![AI/ML](https://img.shields.io/badge/AI%2FML-FF6F00?style=flat-square&logo=tensorflow&logoColor=white) ![IoT](https://img.shields.io/badge/IoT-0088CC?style=flat-square&logo=raspberrypi&logoColor=white) ![Sensors](https://img.shields.io/badge/Sensors-38BDAE?style=flat-square&logoColor=white)

- Leading a **funded incubation project** to develop a Sensory-Aware Wearable Device for physically disabled children
- Integrating **multiple sensor arrays with AI models** to enable real-time environmental awareness and safety monitoring
- Designing **assistive support systems** combining edge computing and IoT for accessibility applications
- Mentored **5 first-year students**, guiding them through Python, AI fundamentals, and collaborative project development

</details>

---

## 🚀 Featured Projects

<div align="center">

| Project | Stack | Highlights |
|:--------|:------|:-----------|
| [🤖 **LUMI Voice AI Assistant**](https://github.com/joelsmith-00) | Python · FastAPI · NLP · Speech | 🏆 3rd Prize – Mindcraft International Tech Expo; Voice-enabled AI assistant with real-time NLP processing; Presented at national technical expo |
| [🌐 **NetSim Vision**](https://github.com/joelsmith-00) | Python · Networking · Simulation · IoT | Full network topology simulator with real-time packet visualization; Supports multiple routing protocol simulations |
| [📄 **Study Buddy PDF Analyzer**](https://github.com/joelsmith-00) | Python · NLP · FastAPI · MongoDB | AI-powered document Q&A system; Extracts insights and answers queries from academic PDFs using NLP pipeline |
| [📡 **Fisherman Border Detection System**](https://github.com/joelsmith-00) | Python · IoT · GPS · AI | Safety system alerting fishermen of maritime border proximity; Combines GPS, IoT sensors, and AI logic |
| [🗄️ **SQL Flow Visualizer**](https://github.com/joelsmith-00) | Python · SQL · Data Viz | Visualizes SQL query execution flow and schema relationships; Educational tool for database learners |
| [🤝 **AI Data Harmony**](https://github.com/joelsmith-00) | Python · ML · Pandas · REST API | Automated data preprocessing and harmonization pipeline; Reduces manual data cleaning effort significantly |

</div>

---

## 🏅 Achievements

<div align="center">

| 🏆 | Achievement | Details |
|:--:|:------------|:--------|
| 🥉 | **Mindcraft International Tech Expo – 3rd Prize** | LUMI Voice AI Assistant; competed nationally against top tech innovators |
| 🚀 | **Smart India Hackathon – Round 2 Qualifier** | Led team through national-level problem statement; advanced to second round |
| 🤖 | **AI for Bharat Hackathon – Round 2 Qualifier** | AI-focused national competition; qualified second round as team leader |
| 🧙 | **Triwizardathon 1.0 – Final Round** | Reached the final round of this multi-disciplinary hackathon |
| 💡 | **AMD Ryzen Slingshot Hackathon – Participant** | Competed in AMD's flagship innovation challenge |
| 🌐 | **GeeksforGeeks Hackathon – Participant** | National coding and product hackathon participant |
| 👨‍🏫 | **Mentored 5 First-Year Students** | Guided juniors in Python, AI, backend dev, and project collaboration on LUMI |
| 🏅 | **Funded Incubation Project** | Selected for institutional incubation funding for assistive wearable AI device |
| 🤾 | **District-Level Handball Player** | Represented district in state competitions; showcases teamwork and athletics |
| 🥋 | **State-Level Wushu Martial Arts Player** | Competed at state level in Wushu; demonstrates discipline and dedication |

</div>

---

## 🎓 Education & Certifications

<div align="center">

| 🎓 Degree | 🏫 Institution | 📅 Year | 📍 Location |
|:----------|:--------------|:--------|:------------|
| **B.Tech – Artificial Intelligence & Data Science** | Karunya Institute of Technology and Sciences | 2022 – 2026 | Tamil Nadu, India |

</div>

<br/>

### 📜 Certifications

<div align="center">

| 🏷️ | Certification | Issuer |
|:--:|:-------------|:-------|
| 🍃 | MongoDB Python Developer Path | MongoDB University |
| ☁️ | AWS Cloud Foundations | Amazon Web Services |
| 📊 | AWS Data Engineering | Amazon Web Services |
| 🤖 | Introduction to AI Concepts | Microsoft |
| 🐍 | Python Essentials | Cisco Networking Academy |
| 🧠 | Generative AI for Beginners | Microsoft / GitHub |
| 📈 | Data Science and Analytics | HP LIFE |
| 🔍 | Data Analytics Job Simulation | Deloitte |

</div>

---

## 🌱 Currently Learning

```
🧱 Large Language Models    →  Fine-tuning · RAG Pipelines · Prompt Engineering
☁️  Cloud-Native AI          →  AWS SageMaker · Lambda · Serverless Deployment
🔬 Deep Learning            →  CNNs · Transformers · Attention Mechanisms
🛠️  MLOps                    →  Model Versioning · CI/CD for ML · Monitoring
📡 Edge AI & IoT            →  TinyML · Embedded AI · Sensor Fusion
```

---

## 📬 Connect With Me

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/joelsmith-00)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/joelsmith-00)
[![Email](https://img.shields.io/badge/Email-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:joelsmith-00@email.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-70A5FD?style=flat-square&logo=vercel&logoColor=white)](https://github.com/joelsmith-00)

</div>

<div align="center">
<i>"Build AI that doesn't just compute — it understands. 🧠"</i>
</div>

---

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=120&section=footer&animation=twinkling" width="100%"/>
